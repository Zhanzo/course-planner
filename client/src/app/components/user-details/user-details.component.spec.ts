...
describe('UserDetailsComponent', () => {
  ...
  beforeEach(async () => {
    ...
    coursePlanServiceSpy = jasmine.createSpyObj('CoursePlanService', ['removeId', 'get', 'storeId', 'delete']);

    userServiceSpy.getCurrentUser.and.returnValue(of(dummyUser));
    coursePlanServiceSpy.get.withArgs(dummyCoursePlans[0].id).and.returnValue(of(dummyCoursePlans[0]));
    coursePlanServiceSpy.get.withArgs(dummyCoursePlans[1].id).and.returnValue(of(dummyCoursePlans[1]));
  ...
  it ('should be able to delete a course plan', fakeAsync(() => {
    ...
    expect(coursePlanServiceSpy.delete).toHaveBeenCalledOnceWith(dummyCoursePlans[0].id);
